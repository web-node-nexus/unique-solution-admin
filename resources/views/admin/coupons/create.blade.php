            ...
            <form method="POST" action="{{ route('admin.coupons.store') }}" enctype="multipart/form-data">
                ...
                <div class="row g-3">
                    <div class="col-md-4">
                        ...
                        <input type="text" name="code" id="code" value="{{ old('code') }}" class="form-control text-uppercase @error('code') is-invalid @enderror" required>
                        @error('code')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-8">
                        <label class="form-label" for="title">Display title</label>
                        <input type="text" name="title" id="title" value="{{ old('title') }}" class="form-control @error('title') is-invalid @enderror" placeholder="Festival Offer">
                    ...
                    <div class="col-md-4">
                        ...
                        <input type="number" step="0.01" min="0" name="discount_value" id="discount_value" value="{{ old('discount_value') }}" class="form-control @error('discount_value') is-invalid @enderror" required>
                        @error('discount_value')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4">
                        <label class="form-label" for="min_order_value">Min order value</label>
                        <input type="number" step="0.01" min="0" name="min_order_value" id="min_order_value" value="{{ old('min_order_value') }}" class="form-control @error('min_order_value') is-invalid @enderror">
                        @error('min_order_value')<div class="invalid-feedback">{{ $message }}</div>@enderror
                    </div>
                    <div class="col-md-4">
                        <label class="form-label" for="max_uses">Max uses</label>
                        <input type="number" min="1" name="max_uses" id="max_uses" value="{{ old('max_uses') }}" class="form-control @error('max_uses') is-invalid @enderror">
