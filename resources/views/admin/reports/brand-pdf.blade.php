<!DOCTYPE html>
<html><head><meta charset="utf-8"><title>Brand Report</title>
...
<h1>Unique Solution — Brand Sales</h1>
<div class="muted">{{ $from->format('d M Y') }} — {{ $to->format('d M Y') }}</div>
<table><thead><tr><th>Brand</th><th>Units</th><th>Revenue</th></tr></thead><tbody>
@foreach($rows as $row)
<tr><td>{{ $row->name ?? 'Unbranded' }}</td><td>{{ $row->units }}</td><td>{{ format_money($row->revenue) }}</td></tr>
@endforeach
